...
use crate::state::CFG;
use crate::tests::utils::{chain_with_mocked_data, default_chain, inst_msg};
use cosmwasm_std::{testing::{mock_dependencies, mock_env, mock_info}, Reply, SubMsgResponse, Event, StdError, Response, Coin};
use cosmwasm_std::{coins, from_binary, to_binary, Attribute, Binary, CosmosMsg, SubMsg, WasmMsg};
...
fn test_delegate() {
    // GIVEN
    let (mut deps, env, info) = chain_with_mocked_data();
...
fn test_delegate_no_funds() {
    // GIVEN
    let (mut deps, env, _info) = chain_with_mocked_data();

use cosmwasm_std::{
    ...
    WasmQuery, QuerierResult, ContractResult, SystemResult, Binary, to_binary, Timestamp
};
use cw721::{AllNftInfoResponse, OwnerOfResponse, NftInfoResponse};
...
use crate::{
    msg::{CW721Collection, InstantiateMsg, ExecuteMsg}, 
    entry_points::{instantiate::instantiate, execute::execute, reply::reply}
};
...
pub fn chain_with_contract_delegation() -> (
    ...
    execute(deps.as_mut(), env.clone(), info.clone(), ExecuteMsg::MsgDelegate {}).unwrap();
    let reply_msg = Reply {
        id : 2,
        result: cosmwasm_std::SubMsgResult::Ok(SubMsgResponse {
            events: vec![],

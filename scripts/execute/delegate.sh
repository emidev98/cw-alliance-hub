...
VALIDATOR_ADDRESS=$(terrad query staking validators -o json | jq -r .validators[0].operator_address)
CONTRACT_ADDR=$(cat ./scripts/contract_addr.txt)

echo "Executing x/alliance MsgDelegate thru $CONTRACT_ADDR on chain..."
terrad tx wasm execute $CONTRACT_ADDR '{"deposit" : {"delegator_address": "'$WALLET_ADDRESS'", "validator_address" :"'$VALIDATOR_ADDRESS'"}}' $TX_ARGS --amount=100uluna | jq 
...
DELEGATE_CODE_RES=$(terrad tx wasm execute $CONTRACT_ADDR '{"msg_delegate": { }}' $TX_ARGS --amount=100$CREATED_RES_DENOM | jq -r .code)

if [ "$DELEGATE_CODE_RES" != "0" ]; then
  echo "Error: Failed to delegate"
  exit 1
fi
...
NFT_CONTRACT_ADDR=$(terrad query wasm contract-state smart $CONTRACT_ADDR '{ "get_config" : {} }' --node=http://localhost:16657 -o json | jq -r .data.nft_contract_addr)

echo $((MINTED_NFTS - 1)) > ./scripts/.metadata/minted_nft_index.txt
echo "MINTED_NFT $((MINTED_NFTS - 1)) NFT_CONTRACT_ADDR $NFT_CONTRACT_ADDR"